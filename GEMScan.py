...
if uploaded_file:
    ...
    frequency_choice = st.sidebar.radio("Select frequency filter", [
        "None", "Exactly 1", "Exactly 2", "Exactly 3", "More than 3"
    ], key="freq_filter")
    if frequency_choice != "None" and freq_column:
        freq_counts = df[freq_column].value_counts()
        if frequency_choice == "Exactly 1":
            selected_values = freq_counts[freq_counts == 1].index
        elif frequency_choice == "Exactly 2":
            selected_values = freq_counts[freq_counts == 2].index
        elif frequency_choice == "Exactly 3":
            selected_values = freq_counts[freq_counts == 3].index
        elif frequency_choice == "More than 3":
            selected_values = freq_counts[freq_counts > 3].index
        freq_filter_condition = df[freq_column].isin(selected_values)
        filter_conditions.append(freq_filter_condition)

    # 🔁 Frequency Filter (by column)
    st.sidebar.markdown("### 🔁 Frequency Filter (by column)")
    freq_column = st.sidebar.selectbox("Column to count instances", df.columns.tolist(), key="freq_col")
    frequency_choice = st.sidebar.radio("Select frequency filter", [
        "Exactly 1", "Exactly 2", "Exactly 3", "More than 3"
    ], key="freq_filter")
    ...
    if freq_column:
        ...
        # Count-based filtering UI
        count_filter_column = st.sidebar.selectbox(
            "Apply frequency filter to which column?",
        ...
        if count_filter_column != "None" and count_filter_mode != "Don't filter":
            val_counts = df[count_filter_column].value_counts()
            if count_filter_mode == "Exactly once":
    ...
    # ✅ Apply uniqueness filter after all other filters
    for ucol in unique_columns:
        if ucol in filtered_df.columns:
            val_counts = filtered_df[ucol].value_counts()
            unique_vals = val_counts[val_counts == 1].index
            filtered_df = filtered_df[filtered_df[ucol].isin(unique_vals)]
            filtered_df = df.copy()
