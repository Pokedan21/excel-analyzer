    ...
    for column in df.columns:
        ...
        if pd.api.types.is_string_dtype(col_data):
            with st.sidebar.expander(f"🔤 Filter: {column}", expanded=False):
                search_term = st.text_input(f"Search {column}", key=f"{column}_search_input")
                if search_term:
                    col_data = col_data[col_data.str.contains(search_term, case=False, na=False)]

                logic_mode_col = st.radio("Match Mode", ["Match ANY (OR)", "Match ALL (AND)"], key=f"{column}_logic_radio")
                unique_values = sorted(col_data.unique().tolist())
                selected_options = st.multiselect(f"{column}", unique_values, key=f"{column}_select_multiselect")
                if selected_options:
